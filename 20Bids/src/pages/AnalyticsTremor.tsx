...
export function AnalyticsTremorPage() {
    ...
    // Theme detection (kept for potential future use)
                ...
                <div className="p-6 space-y-6 bg-bg-primary">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[2.5fr_1fr] lg:grid-cols-[3fr_1fr] gap-6">
                            {/* Performance Evolution Chart - EXACT COPY OF TREMOR EXAMPLE */}
                            {(() => {
                                ...
                                // Build chart data with 3 series like the example
                                const chartData = equityCurve.map(d => ({
                                    date: d.date.slice(5),
                                    'Cumulative': parseFloat(d.equity.toFixed(2)),
                                    'Daily Return': parseFloat(d.return.toFixed(2)),
                                    'Drawdown': parseFloat((d.drawdown || 0).toFixed(2))
                                }));

                                // Summary data for table - matching example structure
                                const summary = [
                                    {
                                        name: 'Take Profit Hits',
                                        value: `${totalHitTP}`,
                                        invested: `${totalTrades > 0 ? ((totalHitTP / totalTrades) * 100).toFixed(1) : 0}%`,
                                        cashflow: `+${(totalHitTP * takeProfit).toFixed(2)}%`,
                                    ...
                                    },
                                    {
                                        name: 'Stop Loss Hits',
                                        value: `${totalHitSL}`,
                                        invested: `${totalTrades > 0 ? ((totalHitSL / totalTrades) * 100).toFixed(1) : 0}%`,
                                        cashflow: `-${(totalHitSL * stopLoss).toFixed(2)}%`,
                                    ...
                                    },
                                    {
                                        name: 'Other Outcomes',
                                        value: `${totalOther}`,
                                        invested: `${totalTrades > 0 ? ((totalOther / totalTrades) * 100).toFixed(1) : 0}%`,
                                        cashflow: '0%',
                                        gain: '0%',
                                        realized: `${totalOther}`,
                                        dividends: 'N/A',
                                                    ...
                                                    <TableRow key={item.name}>
                                                        <TableCell className="font-medium text-tremor-content-strong dark:text-dark-tremor-content-strong">
                                                            <div className="flex space-x-3">
                                                                <span
                                                                    className={cn(item.bgColor, 'w-1 shrink-0 rounded')}
                                                                    aria-hidden={true}
                                                                />
                                                                <span>{item.name}</span>
                                                            </div>
                            ...
                            <div className="w-full h-[260px]">
                                {optimizationData?.bubbleData?.length > 0 ? (() => {
                                    ...
                                    return (
                                        <LineChart
                                            className="h-64"
                                            data={curveData.map((d: any) => ({
                                                sl: `${d.sl}%`,
                                                Efficiency: d.efficiency,
                                                'Win Rate': d.winRate
                                            }))}
                                            index="sl"
                                            categories={["Efficiency", "Win Rate"]}
                                            colors={["blue", "emerald"]}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                            ...
                            {/* Relative Volume Impact - PLOTLY Scatter */}
                            <Card className="p-4">
                                <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="w-full h-[260px]">
                                    {optimizationData?.relVolStats?.length > 0 ? (
                                        <Plot
                                            data={[{
                                                x: optimizationData.relVolStats.slice(0, 200).map((d: any) => d.relativeVol),
                                                y: optimizationData.relVolStats.slice(0, 200).map((d: any) => d.mvso),
                                                mode: 'markers',
                                                type: 'scatter',
                                                marker: { color: '#f59e0b', opacity: 0.5, size: 6 },
                                                hovertemplate: 'Rel Vol: %{x}x<br>MVSO: %{y}%<extra></extra>'
                                            }]}
                                            layout={{
                                                autosize: true,
                                                ...
                                                plot_bgcolor: 'transparent',
                                                xaxis: {
                                                    title: { text: 'Relative Volume', font: { size: 10 } },
                                                    gridcolor: '#f0f0f0'
                                                },
                                                yaxis: {
                                                    title: { text: 'Max Upside %', font: { size: 10 } },
                                                    gridcolor: '#f0f0f0'
                                                }
                                            }}
                                            config={{ displayModeBar: false, responsive: true }}
                        ...
                        {/* ROW 3: Expectancy Distribution (Full Width) - TREMOR */}
                        <Card className="p-4">
                            <div className="flex items-center justify-between mb-3">
                                <Title className="text-xs font-bold uppercase tracking-widest">
